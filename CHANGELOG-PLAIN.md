...
把领域文件搬进了标准的家，并顺手清掉了三个不属于本仓的旧文件。

母模板在 v1.75.0 定了「三层布局」规矩：领域模板仓应该把所有领域自有内容放进根目录一个 `domain/` 文件夹（只有一份例外：领域规则文件必须放在 `ai/` 规则层才能被 AI 自动读到）。本仓原来用的是自己的旧布局 `domain-overlay/`，这版按新规矩搬家：检查表和项目文档骨架进 `domain/scaffold/`，形态说明和文档审计标准进 `domain/standards/`，创建 / 同步剧本改名 `domain/scenarios.md`，两份 README 合并成一份导览。

另一个实质变化是**领域规则的下发方式**：原来两个规则文件会原样复制到每个 agent 项目里（形成第三份规则文件）；新规矩是规则只在模板仓留一份种子（`ai/domain-rules.md`），生成新项目时把规则内容"写进"项目自己的规则文件（`ai/project-rules.md` 第 5 节），项目里不再有独立的领域规则文件。生成脚本已按此改造。

同时清掉三个母模板残留文件（维护者手册、指错方向的 issue 模板、母仓 PR 模板），示例项目也整体搬进新布局。所有自检（0 个发现）、同步预演、示例项目 5 个单元测试全部通过。需要说明：一键创建脚本这版只做了预演级验证，等第一个真实 agent 项目时再端到端跑一次。
